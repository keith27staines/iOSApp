...
static const NSUInteger kGMUClusterDistancePoints = 70; //100
...
- (NSArray<id<GMUCluster>> *)clustersAtZoom:(float)zoom {
  ...
  NSMutableSet<id<GMUClusterItem>> *processedItems = [[NSMutableSet alloc] init];
    ...
    //get the selected item + remove from the list
    id<GMUClusterItem> selectedItem = NULL;
    for (int i = 0; i < _items.count; i++)
    {
        id<GMUClusterItem> item = _items[i];
        if (item.shouldShowView)
        {
            selectedItem = item;
        }
    }
    if(selectedItem != NULL)
    {
        ...
        GMUClusterItemQuadItem *quadItem = [[GMUClusterItemQuadItem alloc] initWithClusterItem:selectedItem];
        // This should remove the corresponding quad item since GMUClusterItemQuadItem forwards its hash
        // and isEqual to the underlying item.
        [_quadTree remove:quadItem];
    }
  ...
  for (id<GMUClusterItem> item in _items) {
    ...
    //add a new cluster with the selected point
    if(selectedItem != NULL)
    {
        [_items addObject:selectedItem];
        GMUStaticCluster *cluster = [[GMUStaticCluster alloc] initWithPosition:selectedItem.position];
        GMUWrappingDictionaryKey *key = [[GMUWrappingDictionaryKey alloc] initWithObject:selectedItem];
        NSNumber *number = [NSNumber numberWithDouble:0];
        [itemToClusterDistanceMap setObject:number forKey:key];
        [itemToClusterMap setObject:cluster forKey:key];
        [cluster addItem:selectedItem];
        [clusters addObject:cluster];
    }
