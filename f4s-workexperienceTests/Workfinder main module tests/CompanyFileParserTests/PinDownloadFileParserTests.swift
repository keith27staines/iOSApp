...
class PinDownloadFileParserTests: XCTestCase {
    ...
    func test_jsonifiedString() {
        XCTAssertEqual(sut.replaceTerminatingSquareBrackets(lineString: _line2), "{\"companyUuid1\",1.01,2.02,[\"tagUuid1\",\"tagUuid2\"]}")
        XCTAssertEqual(sut.insertLatitudeLongitudeKeys(lineString: _line2), "[\"companyUuid1\",\"lat\":1.01,\"lon\":2.02,\"tags\":[\"tagUuid1\",\"tagUuid2\"]]")
        XCTAssertEqual(sut.jsonifiedString(from: _line2), "{\"companyUuid\":\"companyUuid1\",\"lat\":1.01,\"lon\":2.02,\"tags\":[\"tagUuid1\",\"tagUuid2\"]}")
    }
    
    func test_jsonFromLineString() {
        let json = sut.pinJsonFromLineString(lineString: _line2)
        XCTAssertEqual(json.companyUuid,"companyUuid1")
